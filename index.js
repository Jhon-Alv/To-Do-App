...
function showMenu(selectedTask) {
  ...
  const closeMenu = (e) => {
    if (e.target.tagName != "I" && !menuDiv.contains(e.target)) {
      menuDiv.classList.remove("show");
      document.removeEventListener("click", closeMenu); // Elimina el listener después de cerrar
    }
  };

  document.addEventListener("click", closeMenu); // Agrega el listener para cerrar el menú
}
